...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/csjmu/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating csjmu Identities"
  ...
  infoln "Generating CCP files for csjmu"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.csjmu.com docker_peer0.csjmu.com docker_peer1.csjmu.com docker_peer2.csjmu.com docker_peer3.csjmu.com docker_peer4.csjmu.com 
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/csjmu/msp organizations/fabric-ca/csjmu/tls-cert.pem organizations/fabric-ca/csjmu/ca-cert.pem organizations/fabric-ca/csjmu/IssuerPublicKey organizations/fabric-ca/csjmu/IssuerRevocationPublicKey organizations/fabric-ca/csjmu/fabric-ca-server.db'
